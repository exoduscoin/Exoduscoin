...
uint256 hashGenesisBlock("0x02cfb2f6cccdb0da1d91c2c494ebe877431cdd4c2adabddc7200965b9e14422d");
...
const string strMessageMagic = "ExodusCoin Signed Message:\n";
...
{
    // ExodusCoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
static const int64 nTargetSpacing = 20 * 60;
...
{
    // ExodusCoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "exodus genesis";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
	    ...
	    assert(block.hashMerkleRoot == uint256("36f022cb6bdc25ae07608638e465ab4cc3f8f34185346aa607f9790244eda278"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xd2, 0xce, 0xaf, 0xeb };
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("ExodusCoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("ExodusCoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static ExodusCoinMiner(CWallet *pwallet)
{
    printf("ExodusCoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("ExodusCoin-miner");
    ...
    try { loop {
        ...
        printf("Running ExodusCoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("ExodusCoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&ExodusCoinMiner, pwallet));
}
