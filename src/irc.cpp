...
// Copyright (c) 2011-2012 ExodusCoin Developers
...
void ThreadIRCSeed()
{
    ...
    RenameThread("ExodusCoin-ircseed");
...
void ThreadIRCSeed2()
{
    ...
    while (true)
    {
        ...
        if (fTestNet) {
            Send(hSocket, "JOIN #ExodusCoinTEST0\r");
            Send(hSocket, "WHO #ExodusCoinTEST0\r");
        } else {
            // randomly join #ExodusCoin00-#ExodusCoin99
            // network is now over 3k peers , get them to join 50 random channels!
            //            channel_number = 0; 
            int channel_number = 0; //GetRandInt(50);

            Send(hSocket, strprintf("JOIN #ExodusCoin%02d\r", channel_number).c_str());
            Send(hSocket, strprintf("WHO #ExodusCoin%02d\r", channel_number).c_str());
        }
